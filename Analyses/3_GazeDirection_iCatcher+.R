...
# Preparation
d.i.n.2 <- d.i.n %>% subset(!Prop.face==0) %>% 
...
d.i.n.3 <- d.i.n %>% subset(!Prop.face==0) %>% 
  mutate(N.all=right+left+away) %>% 
...
d.i.a.2 <- d.i.a %>% subset(!Prop.face==0) %>% 
  mutate(N.all=right+left+away) %>% 
  pivot_longer(c(away, right, left, noface), names_to="Category", values_to="N.correct") %>% 
  mutate(tmp=str_replace_all(Category, c("^left$"="_right", "^right$"="_left", "^away$"="away", "^noface$"="noface")),
         Prediction=str_replace_all(tmp, c("^_left$"="left", "^_right$"="right")),
...
d.i.a.3 <- d.i.a %>% subset(!Prop.face==0) %>% 
  mutate(tmp=str_replace_all(Annotation, c("^left$"="_right", "^right$"="_left", "^away$"="away", "^noface$"="noface")),
         Prediction=str_replace_all(tmp, c("^_left$"="left", "^_right$"="right")),
         Actual=case_when(trial==4|trial==5~"left", trial==6|trial==7~"right", T~"away"),
         Y=case_when(Actual==Prediction~1, T~0)) %>% select(-tmp)
...
# Descriptive stats
d.i.n %>% subset(Prop.face==0) %>% select(filename) # NA
d.i.a %>% subset(Prop.face==0) %>% select(filename) # c05_Lr_D30_Sr24_Rr_09_1.txt

d.i.n.2 %>% summarize(N = n(), M.Prop = mean(Prop), SD.Prop=sd(Prop)) # Non-Anonymized
d.i.a.2 %>% summarize(N = n(), M.Prop = mean(Prop), SD.Prop=sd(Prop)) # Anonymized


# -----------------------------------------------------------------------------------------
# Regression modeling (Non-Anonymized)

fit.g.i.n <- glmer(cbind(N.correct, N.all-N.correct) ~ Lighting + Distance + Position + Rotation + Country + (1|id), data=d.i.n.2, family=binomial,
                   # control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(fit.g.i.n)
Anova(fit.g.i.n)
tmp <- summary(fit.g.i.n)$coefficients %>% as.data.frame() %>% mutate(Monitor="Small", Anonym="Non-anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
set.seed(123)
tmp1 <- data.frame(Random=sample(1:3, nrow(d.i.n.2), replace=TRUE)) %>%
  cbind(dplyr::select(d.i.n.2, Actual)) %>%
...
## Visualization
d.i.n.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  ...
  geom_pointrange(data=res.g.i.n, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Non-anonymized, Small monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
...
print(gp)
ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Small.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.n.3 %>% subset(Lighting=="Front" & Distance=="Close\n(30 cm)" & Position=="Left (24 cm)" & Rotation=="Center")
### Worst condition
d.spec <- d.i.n.3 %>% subset(Lighting=="Left" & Distance=="Middle\n(60 cm)" & Position=="Right (24 cm)" & Rotation=="Left")
...
d.i.n.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Small_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Small_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Small_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Small_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)


# -----------------------------------------------------------------------------------------
# Regression modeling (Anonymized)

fit.g.i.a <- glmer(cbind(N.correct, N.all-N.correct) ~ Lighting + Distance + Position + Rotation + Country + (1|id), data=d.i.a.2, family=binomial,
                   # control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(fit.g.i.a)
Anova(fit.g.i.a)
tmp <- summary(fit.g.i.a)$coefficients %>% as.data.frame() %>% mutate(Monitor="Small", Anonym="Anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
## Visualization
d.i.a.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  summarize(ncrr_prop=mean(Prop)) %>% ungroup() %>%
  mutate(Position=factor(Position, levels=c("Sl24"="Left (24 cm)", "Sc00"="Center", "Sr24"="Right (24 cm)")),
         Lighting=factor(Lighting, levels=c("Left", "Front", "Right")),
         Rotation=str_replace_all(Rotation, c("^Center$"="Upright")),
  ...
  # geom_abline(slope=0, intercept=0.33, linetype="dashed", color="gray30", lwd=0.5)+
  geom_jitter(aes(color=Rotation), position=position_jitterdodge(jitter.width=0.3, jitter.height=0.05), size=0.5, alpha=0.15)+
  geom_pointrange(data=res.g.i.a, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Anonymized, Small monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
  facet_grid(Distance~Position)+
  theme(plot.title=element_text(size=16,face="bold", color="black"),
        axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Small.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.a.3 %>% subset(Lighting=="Front" & Distance=="Close\n(30 cm)" & Position=="Left (24 cm)" & Rotation=="Center")
### Worst condition
d.spec <- d.i.a.3 %>% subset(Lighting=="Left" & Distance=="Middle\n(60 cm)" & Position=="Center" & Rotation=="Left")
### Creating Confusion matrix
tmp1 <- d.spec %>% group_by(Actual) %>% summarize(Nact=n())
tmp2 <- d.spec %>% group_by(Prediction) %>% summarize(Npred=n())
...
d.i.a.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Small_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Small_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Small_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Small_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)
...
summary(fit.g.i.n)
Anova(fit.g.i.n)
tmp <- summary(fit.g.i.n)$coefficients %>% as.data.frame() %>% mutate(Monitor="Medium", Anonym="Non-anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
d.i.n.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  ...
  geom_pointrange(data=res.g.i.n, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Non-anonymized, Medium monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
  facet_grid(Distance~Position)+
  theme(plot.title=element_text(size=16,face="bold", color="black"),
        axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
ggsave(file=here("Figures", "Figure_3_iCatcher+_NonAnonym_Medium.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.n.3 %>% subset(Lighting=="Front" & Distance=="Close\n(30 cm)" & Position=="Center" & Rotation=="Center")
### Worst condition
d.spec <- d.i.n.3 %>% subset(Lighting=="Left" & Distance=="Far\n(90 cm)" & Position=="Right (24 cm)" & Rotation=="Right")
### Creating Confusion matrix
...
## Summary stats
d.i.n.mat %>% subset(Actual==Prediction) %>% ungroup() %>% summarize(Accuracy=sum(N)/sum(Nact)) # overall accuracy
d.i.n.mat %>% subset(Actual==Prediction) %>% ungroup() %>% summarize(Fmean=mean(Fmeasure, na.rm=TRUE)) # overall F measure (averaged)
...
d.i.n.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_3_iCatcher+_NonAnonym_Medium_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_3_iCatcher+_NonAnonym_Medium_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Medium_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Medium_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)
...
summary(fit.g.i.a)
Anova(fit.g.i.a)
tmp <- summary(fit.g.i.a)$coefficients %>% as.data.frame() %>% mutate(Monitor="Medium", Anonym="Anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
d.i.a.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  ...
  geom_pointrange(data=res.g.i.a, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Anonymized, Medium monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
  facet_grid(Distance~Position)+
  theme(plot.title=element_text(size=16,face="bold", color="black"),
        axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Medium.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.a.3 %>% subset(Lighting=="Front" & Distance=="Close\n(30 cm)" & Position=="Center" & Rotation=="Center")
### Worst condition
d.spec <- d.i.a.3 %>% subset(Lighting=="Left" & Distance=="Far\n(90 cm)" & Position=="Right (24 cm)" & Rotation=="Left")
### Creating Confusion matrix
tmp1 <- d.spec %>% group_by(Actual) %>% summarize(Nact=n())
tmp2 <- d.spec %>% group_by(Prediction) %>% summarize(Npred=n())
d.i.a.mat <- d.spec %>% group_by(Actual, Prediction) %>% summarize(N=n()) %>% 
...
d.i.a.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Medium_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Medium_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Medium_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Medium_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)
...
summary(fit.g.i.n)
Anova(fit.g.i.n)
tmp <- summary(fit.g.i.n)$coefficients %>% as.data.frame() %>% mutate(Monitor="Large", Anonym="Non-anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
d.i.n.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  ...
  geom_pointrange(data=res.g.i.n, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Non-anonymized, Large monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
  facet_grid(Distance~Position)+
  theme(plot.title=element_text(size=16,face="bold", color="black"),
        axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Large.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.n.3 %>% subset(Lighting=="Left" & Distance=="Close\n(30 cm)" & Position=="Center" & Rotation=="Center")
### Worst condition
d.spec <- d.i.n.3 %>% subset(Lighting=="Front" & Distance=="Far\n(90 cm)" & Position=="Right (24 cm)" & Rotation=="Right")
### Creating Confusion matrix
tmp1 <- d.spec %>% group_by(Actual) %>% summarize(Nact=n())
tmp2 <- d.spec %>% group_by(Prediction) %>% summarize(Npred=n())
d.i.n.mat <- d.spec %>% group_by(Actual, Prediction) %>% summarize(N=n()) %>% 
...
d.i.n.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Large_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_S2_iCatcher+_NonAnonym_Large_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Large_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_NonAnonym_Large_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)
...
summary(fit.g.i.a)
Anova(fit.g.i.a)
tmp <- summary(fit.g.i.a)$coefficients %>% as.data.frame() %>% mutate(Monitor="Large", Anonym="Anonymized") %>% 
  rownames_to_column(var="Term")
coef.i <- coef.i %>% rbind(tmp)
...
d.i.a.2 %>% group_by(id, Lighting, Distance, Position, Rotation, Country) %>% 
  ...
  geom_pointrange(data=res.g.i.a, aes(y=predicted, ymin=conf.low, ymax=conf.high, color=Rotation, shape=Rotation), position=position_dodge(0.7), size=0.5, linewidth=0.8)+
  scale_y_continuous(limits=c(-0.05,1.05), breaks=seq(0,1,0.25))+
  labs(title="iCatcher+ (Anonymized, Large monitor size)", x="Lighting", y="Proportion of correct prediction")+
  theme_bw()+
  facet_grid(Distance~Position)+
  theme(plot.title=element_text(size=16,face="bold", color="black"),
        axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Large.png"), plot=gp, dpi=350, width=7.7, height=6.5)

## Confusion matrix
### Best condition 
d.spec <- d.i.a.3 %>% subset(Lighting=="Front" & Distance=="Close\n(30 cm)" & Position=="Center" & Rotation=="Center")
### Worst condition
d.spec <- d.i.a.3 %>% subset(Lighting=="Right" & Distance=="Far\n(90 cm)" & Position=="Right (24 cm)" & Rotation=="Right")
### Creating Confusion matrix
tmp1 <- d.spec %>% group_by(Actual) %>% summarize(Nact=n())
tmp2 <- d.spec %>% group_by(Prediction) %>% summarize(Npred=n())
d.i.a.mat <- d.spec %>% group_by(Actual, Prediction) %>% summarize(N=n()) %>% 
...
d.i.a.mat %>% ggplot(aes(x=Prediction, y=Actual))+
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Large_CM_Best.png"), plot=gp, dpi=350, width=4.2, height=3)
ggsave(file=here("Figures", "Figure_S3_iCatcher+_Anonym_Large_CM_Worst.png"), plot=gp, dpi=350, width=4.2, height=3)
...
d.spec %>% group_by(trial, Prediction) %>% summarize(Y.sum=n()) %>% left_join(tmp, by="trial") %>% 
  ...
  theme(axis.ticks=element_line(color = "black"),
        ...
        legend.text=element_text(size=12, color="black")) -> gp
print(gp)
# ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Large_Best.png"), plot=gp, dpi=350, width=5.2, height=4)
ggsave(file=here("Figures", "Figure_S6_iCatcher+_Anonym_Large_Worst.png"), plot=gp, dpi=350, width=5.2, height=4)
